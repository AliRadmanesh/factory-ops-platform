...
The system SHALL connect to Supabase from the local dev environment using `SUPABASE_URL` and `SUPABASE_ANON_KEY` environment variables defined in `.env.local`. A test query to any Supabase table SHALL succeed without error.
...
- **THEN** at least 5 rows are returned (Raw Materials, Fabrication, Assembly, Quality Control, Dispatch)
...
The Vercel-deployed app SHALL successfully query Supabase in production. Environment variables (`SUPABASE_URL`, `SUPABASE_ANON_KEY`) SHALL be configured in Vercel project settings.
